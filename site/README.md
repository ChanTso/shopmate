...
All four buyer clips are based on actual iOS Simulator application frames from the local demo on 2026-09-11. They show real model recommendations, checkout and mock-payment confirmations, persisted memory editing, and iPad shopping alongside chat. The clips are muted, inline H.264, with WebP posters. Inactive/offscreen clips pause. Both pause controls and the reduced-motion preference stop automatic playback.

These are edited product demonstrations, not latency or frame-rate evidence: idle waits are shortened and some completed frames are held. The phone and wide-screen input animations reverse and retimes a recording of native per-character deletion, producing a readable typing sequence without replacing the native screen. The requests, response content and final states come from the actual application. The memory reply uses a post-produced per-character reveal over its recorded final text.
...
- Memory: `memory-chat-input-raw.mp4` ranges `25–34` then `4–13`, each reversed at 4×, show native assistant input. `memory-chat-flow-raw.mp4` ranges `12–16` retimed to 32.5% and `39–43.5` show the actual request and remembered-preference response. The complete native reply at approximately `41.13` is revealed character by character in place during `7.833–9.233` of the edited clip. This 1.4-second presentation animation has an individual stage for each of the 19 characters and punctuation marks, with short punctuation pauses. These added intermediate frames are post-production, not native streaming capture or latency evidence; the original response content and the rest of the recorded UI sequence are retained. `memory-raw.mp4` ranges `0–8`, `19–29`, `37–42` continue the same preference scenario into My, viewing, editing and reloading the saved preference.
...
Chinese display headings and chapter names use locally bundled Noto Serif SC; Latin display text and chapter numbers use Cormorant Garamond. Body and control fonts, CSS weights and sizes remain unchanged. Chapter numbers and names share the existing fixed baseline; source files, subsetting scope and licenses are recorded in `assets/fonts/`. Each progress track fills within its own chapter column. Settled buyer chapters occupy roughly 1.55–2 viewport heights of scrolling. The introductory device transfer spans 1.15 viewport heights; text handovers span 0.6 and wide-screen expansion spans 1.1, all driven by scroll position. On wide-screen expansion, labels move beside their numerals, with navigation above the centered description and device. The merchant heading and description precede a separate chapter row and fixed-size workspace. Chapter buttons position the page immediately, without a playback wait or timed transition. Header and introductory anchor transitions use an interruptible 2.2–2.6-second entrance; ordinary wheel scrolling remains native. The merchant growth animation uses a 2.4-second ease-out cubic curve on the animation frame clock.
