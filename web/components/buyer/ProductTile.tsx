...
import { flyToCart } from "@/lib/buyer-flight";
import { attributeChips, productGlyph, productTileClass } from "@/lib/buyer-format";
...
export function AddButton({
  ...
  return (
    <button
      type="button"
      disabled={!chat?.ready || chat.busy || phase === "busy"}
      onClick={async (event) => {
        event.stopPropagation();
        ...
        setPhase("busy");
        let added = false;
        try { added = (await onAdd(product)) !== false; } catch { added = false; }
