...
import { buyerApi, fetchProduct } from "@/lib/buyer-api";
import type { PriceIntelligence, Product, ProductDetails, ProductsPayload, ReviewAspects } from "@/lib/buyer-types";
...
function VariantList({ family, variants, onAdd }: { family: Product; variants: Product[]; onAdd?: (product: Product) => boolean | void | Promise<boolean | void> }) {
      ...
      {variants.map((variant) => {
        ...
        return (
          <li key={variant.product_id}>
            <button
              type="button"
              disabled={!available || !chat?.ready || chat.busy}
              onClick={() => { if (onAdd) void onAdd(variant); else ask(`将 ${family.title} 的 ${label}（${variant.product_id}）加入购物车。`); }}
...
export function ProductDetail({
  ...
  useEffect(() => {
    let mounted = true;
    const previousPage = buyerApi.page;
    buyerApi.page = {page_type:"product",product_id:product.product_id};
    setDetails(null); setError(null);
    void fetchProduct(product.product_id).then((value) => {
      if (mounted) setDetails(value);
    }).catch(error => { if (mounted) setError(error instanceof Error ? error.message : "无法读取商品详情"); });
    return () => {
      mounted = false;
      if (buyerApi.page.product_id === product.product_id) buyerApi.page = previousPage;
    };
  }, [product.product_id]);
  ...
  return (
    <div className="ac-reveal mb-1 mt-3 rounded-xl border border-(--line) bg-(--well)/40 p-3">
      ...
      {error ? <p role="alert" className="mt-2 text-sm text-(--danger)">{error}</p> : details === null ? (
        ...
        <div className="ac-reveal">
          ...
          {details.variants?.length ? <VariantList family={details} variants={details.variants} onAdd={onAdd} /> : null}
