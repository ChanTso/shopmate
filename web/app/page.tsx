...
function Workbench({ selected, subject, sessions, sessionBusy, error, onSelect, onCreate, onReload, onLogout, onBusy }: { selected: SessionDetail; subject: string; sessions: SavedSession[]; sessionBusy: boolean; error: string | null; onSelect: (id: string) => void; onCreate: () => void; onReload: () => void; onLogout: () => void; onBusy: (busy: boolean) => void }) {
  ...
  return <><PortalShell brand={{ mark: <StoreMark />, name: "ShopMate", detail: "商家经营工作台" }} nav={nav} view={view} onViewChange={setView} operator={{ name: subject, role: "商家操作员" }} assistantOpen={assistantOpen} assistantBusy={busy} onToggleAssistant={() => setAssistantOpen(x => !x)} rail={<AssistantRail open={assistantOpen} storageKey="shopmate-panel-width" onClose={() => setAssistantOpen(false)}>{rail => <AssistantPanel chat={{ ...chat, send, actOnChange, ready: chat.ready && !busy && !sessionBusy }} actionsDisabled={busy || sessionBusy} prefill={prefill} onPrefill={ask} newMemoryCount={0} onOpenActivity={() => setActivityOpen(true)} {...rail} />}</AssistantRail>}>
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-(--line) bg-(--card) p-3"><label htmlFor="session" className="text-xs text-(--ink-soft)">会话</label><select id="session" value={selected.session_id} onChange={e => onSelect(e.target.value)} disabled={busy || sessionBusy} className="min-w-0 max-w-xs flex-1 rounded-md bg-(--ground) px-2 py-2 text-sm disabled:opacity-50">{sessions.map(s => <option key={s.session_id} value={s.session_id}>{!s.title || s.title === "New conversation" ? "新会话" : s.title} · {new Date(s.updated_at).toLocaleDateString("zh-CN")}</option>)}</select><Button variant="secondary" size="sm" disabled={busy || sessionBusy} onClick={onCreate}>新会话</Button><Button variant="secondary" size="sm" disabled={chat.busy || actionBusy || sessionBusy} onClick={onReload}>刷新会话</Button><Button variant="secondary" size="sm" disabled={busy || sessionBusy} onClick={onLogout}>退出</Button></div>
