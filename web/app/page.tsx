...
import { AssistantRail, Button, Notice, type PortalNavItem, PortalShell, type Prefill, Sheet, useMerchantChat, useResource } from "web-shared";
...
type View = "home" | "catalog" | "changes";
...
function Workbench({ selected, subject, sessions, sessionBusy, error, onSelect, onCreate, onReload, onLogout, onBusy }: { selected: SessionDetail; subject: string; sessions: SavedSession[]; sessionBusy: boolean; error: string | null; onSelect: (id: string) => void; onCreate: () => void; onReload: () => void; onLogout: () => void; onBusy: (busy: boolean) => void }) {
  ...
  const busy = chat.busy || selected.status === "running";
  useEffect(() => { onBusy(busy); return () => onBusy(false); }, [busy, onBusy]);
  useEffect(() => { setAssistantOpen(window.innerWidth >= 1024); }, []);
  const { data: overview, failed } = useResource(fetchOverview, [refreshKey]);
  const ask = useCallback((text: string) => { setAssistantOpen(true); setPrefill({ text, nonce: Date.now() }); }, []);
  const nav: PortalNavItem<View>[] = [{ id: "home", label: "经营概览", icon: "home" }, { id: "catalog", label: "商品", icon: "tag" }, { id: "changes", label: "草案与审批历史", icon: "edit", count: overview?.needs_attention.pending_changes.length }];
  return <><PortalShell brand={{ mark: <StoreMark />, name: "ShopMate", detail: "商家经营工作台" }} nav={nav} view={view} onViewChange={setView} operator={{ name: subject, role: "商家操作员" }} assistantOpen={assistantOpen} assistantBusy={busy} onToggleAssistant={() => setAssistantOpen(x => !x)} rail={<AssistantRail open={assistantOpen} storageKey="shopmate-panel-width" onClose={() => setAssistantOpen(false)}>{rail => <AssistantPanel chat={{ ...chat, busy: busy || sessionBusy, ready: chat.ready && !sessionBusy }} prefill={prefill} onPrefill={ask} newMemoryCount={0} onOpenActivity={() => setActivityOpen(true)} {...rail} />}</AssistantRail>}>
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-(--line) bg-(--card) p-3"><label htmlFor="session" className="text-xs text-(--ink-soft)">会话</label><select id="session" value={selected.session_id} onChange={e => onSelect(e.target.value)} disabled={busy || sessionBusy} className="min-w-0 max-w-xs flex-1 rounded-md bg-(--ground) px-2 py-2 text-sm disabled:opacity-50">{sessions.map(s => <option key={s.session_id} value={s.session_id}>{!s.title || s.title === "New conversation" ? "新会话" : s.title} · {new Date(s.updated_at).toLocaleDateString("zh-CN")}</option>)}</select><Button variant="secondary" size="sm" disabled={busy || sessionBusy} onClick={onCreate}>新会话</Button><Button variant="secondary" size="sm" disabled={chat.busy || sessionBusy} onClick={onReload}>刷新会话</Button><Button variant="secondary" size="sm" disabled={busy || sessionBusy} onClick={onLogout}>退出</Button></div>
    ...
    {view === "home" && <HomeView data={overview} failed={failed} onAskAssistant={ask} onReview={() => setView("changes")} />}
    {view === "catalog" && <CatalogView refreshKey={refreshKey} onAskAssistant={ask} />}
    {view === "changes" && <ChangesView data={overview} failed={failed} onAct={busy || sessionBusy ? undefined : chat.actOnChange} />}
  </PortalShell>{activityOpen && <Sheet title="本次连接的运行记录" onClose={() => setActivityOpen(false)} closeLabel="关闭运行记录"><p className="text-xs text-(--ink-soft)">显示当前连接内的工具调用和耗时；历史会话从聊天区恢复。</p>{chat.trace.length ? chat.trace.map((entry, i) => <details key={i} className="rounded-xl border border-(--line) p-3 text-xs"><summary className="cursor-pointer font-medium">{entry.label}{entry.elapsedMs != null ? ` · ${(entry.elapsedMs / 1000).toFixed(1)} 秒` : ""}</summary><pre className="mt-2 whitespace-pre-wrap break-all text-(--ink-soft)">{entry.detail}</pre></details>) : <p className="text-sm text-(--ink-soft)">本次连接还没有工具调用。</p>}</Sheet>}</>;
}
