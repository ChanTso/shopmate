...
The merchant entry point is React/Vite Web at `/`; buyers use the [Kotlin/Compose Android app](../android/README.md) or [SwiftUI iOS app](../ios/README.md). See the [buyer guide](BUYER.md) for sign-in, explicit confirmation, stopping, recovery, and memory management. The old `/buyer` page, support-agent entry point, and duplicate model loop have been retired; Java authorization, refund confirmation, and receipt mechanisms remain in use. Both roles can use web search with sources; business analysis can use an independent Python sandbox. The [historical retail task, UI, and boundary record](../evals/records/retail-v1-20260907/README.md) covers memory, concurrency, and interruption recovery at its recorded versions. The [final retail acceptance](../evals/records/retail-v2-20260907/README.md) retains its separate versions and attempt counts.

## Agent execution

1. The buyer or merchant agent starts a model request with the role's tools, conversation context, and on-demand Skills. The [buyer loop](../vendor/commerce-agents/shopping-agent/runtime-messages-api/shopping_agent_runtime/orchestrator.py) and [merchant loop](../vendor/commerce-agents/merchant-agent/runtime-messages-api/merchant_agent_runtime/orchestrator.py) own the model rounds.
2. The [Chat adapter](../src/shopmate/providers/chat_to_messages.py) streams text incrementally and assembles tool argument fragments. Before closing any tool block, it waits for the complete upstream response and checks the finish reason, nonempty and unique call IDs, tool names, and JSON-object arguments. A truncated or malformed tool request fails before dispatch; text can still appear while the response streams.
3. [Tool dispatch and result collection](../vendor/commerce-agents/commerce-common/commerce_common/turn.py) associate concurrent outcomes with their call IDs. [Cart gates](../vendor/commerce-agents/shopping-agent/core/shopping_agent/gates.py) serialize cart read-modify-write operations within a conversation. Results return to the model for the next round, while presentation events update the client.
4. Context trimming clears old tool-result bodies while preserving call/result pairing. The [shared budget](../src/shopmate/provider.py) bounds model calls and elapsed time across main and delegated work; stopping or expiry cancels in-flight work. Sandbox cleanup and the configured limits are described under [models, budgets, and time](#models-budgets-and-time).

The current Chat path dispatches tools only after validating the complete upstream response; it does not overlap tool execution with the remaining upstream generation. This follows from the adapter's delayed block closure, independently of the dispatcher flag.

Relevant regression tests cover [protocol fragments and completion](../tests/test_chat_to_messages.py), [shared budgets and in-flight cancellation](../tests/test_provider_budget.py), and [parallel call ownership](../tests/test_buyer_persistence.py).
...
[Conversation writes](../src/shopmate/sessions.py) use version-based compare-and-swap checks to reject stale snapshots; restart marks unfinished generation as interrupted. Separately, [memory edits and deletion](../src/shopmate/memory.py) advance a user-and-role memory revision, rejecting a late write from an older turn. These protect different state: saved conversation history and editable preferences. See the [conversation](../tests/test_sessions.py) and [memory revision](../tests/test_buyer_persistence.py) regressions.
...
The operator account is `shopmate-fixture-operator`; its locally generated password is in ignored `.run/operator_password`. Bearer tokens stay in page memory, so refreshing requires sign-in again. See the [Android](../android/README.md) and [iOS](../ios/README.md) guides for buyer builds and installation. Android Emulator connects to `http://10.0.2.2:8101`; iOS Simulator defaults to `http://localhost:8101`.
...
Web search uses a separate Responses request behind the existing ordinary tool interface. It returns external summaries, actual citations, and provider-supplied consulted sources. Source cards distinguish citations from consulted pages and explicitly indicate missing metadata. Web content is not authoritative for this store's products, orders, policies, or permissions. The current proxy does not support native Messages server tools, so this deployment does not enable native server search or code execution; the host performs search and Python execution. Tool dispatch follows the complete-response validation described in [agent execution](#agent-execution). See the [Responses web-search documentation](https://developers.openai.com/api/docs/guides/tools-web-search) for the fields.
