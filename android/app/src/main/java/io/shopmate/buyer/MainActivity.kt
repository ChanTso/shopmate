...
fun BuyerApp(vm: BuyerViewModel = viewModel()) {
    ...
    val pages =
        listOf(
            ...
            "助手" to Icons.Outlined.AutoAwesome,
            "购物车" to Icons.Outlined.ShoppingBag,
            "订单" to Icons.Outlined.ReceiptLong,
            ...
            Column(Modifier.weight(1f)) {
                Row(
                    Modifier.fillMaxWidth()
                        .padding(horizontal = 20.dp, vertical = if (expanded) 4.dp else 12.dp),
                    ...
                    Column {
                        Text(
                            ...
                            fontFamily = FontFamily.Serif,
                            fontWeight = FontWeight.Bold,
                            fontSize = if (expanded) 24.sp else 29.sp,
                        )
                        if (!expanded) Note("精选生活 · 官方商店")
                    }
                    IconButton(
                        onClick = {
                            when (state.screen) {
                                "助手" -> vm.restoreChat()
                                "我的" -> vm.refreshProfile()
                                else -> vm.refresh()
                            }
                        }
                    ) {
                ...
                if (state.loading || state.writing)
                    LinearProgressIndicator(Modifier.fillMaxWidth(), color = Vermilion)
                    ...
                    Row(Modifier.fillMaxSize()) {
                        Box(Modifier.weight(1f)) {
                            when (state.screen) {
                                ...
                                "订单" ->
                                    OrdersScreen(vm, state) { label, action ->
                                        confirm = label to action
                                    }
    ...
    confirm?.let { (message, action) ->
        AlertDialog(
            ...
            dismissButton = { TextButton(onClick = { confirm = null }) { Text("再看一下") } },
        )
    }
}

@Composable
fun LoginScreen(vm: BuyerViewModel, state: BuyerState) {
    ...
    Column(
        ...
        Button(
            ...
            Text(if (state.loading) "登录中…" else "登录，开始探索")
        }
    ...
    LazyColumn(
        ...
        item {
            Row(
                ...
                listOf("配一套咖啡角装备", "比较两款咖啡机", "按预算做购物计划").forEach { text ->
                    SuggestionChip(
                        onClick = {
                            vm.navigate("助手")
                            vm.draft(text)
                ...
                Surface(color = Ink, shape = RoundedCornerShape(24.dp)) {
                    Column(
                        ...
                        Button(
                            onClick = {
                                vm.navigate("助手")
                                vm.draft("预算2000元，帮我规划一套家庭咖啡角装备")
                            }
                        ) {
                            Text("开始规划  →")
                        }
        ...
        if (state.nextOffset != null)
            item {
                OutlinedButton(
                    onClick = { vm.search(state.query, true) },
...
@Composable
fun ProductTile(product: Product, vm: BuyerViewModel) {
    Column(
        Modifier.fillMaxWidth()
            .background(MaterialTheme.colorScheme.surface, RoundedCornerShape(18.dp))
            .border(1.dp, Line, RoundedCornerShape(18.dp))
            .clickable { vm.openProduct(product.product_id) }
    ) {
        ProductImage(
            vm.api.asset(product.image_url),
            product.title,
            Modifier.fillMaxWidth()
                .aspectRatio(1.1f)
                .background(Line, RoundedCornerShape(topStart = 18.dp, topEnd = 18.dp)),
        )
        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(7.dp)) {
            Text(product.title, fontSize = 15.sp, minLines = 2, maxLines = 3)
            Text(
                decimalMoney(product.price, product.currency),
                color = Vermilion,
                fontWeight = FontWeight.Bold,
                fontSize = 20.sp,
            )
...
fun ProductDialog(product: Product, vm: BuyerViewModel, writing: Boolean) {
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            ProductImage(
                vm.api.asset((chosen ?: product).image_url),
                product.title,
                Modifier.fillMaxWidth().height(240.dp),
            )
            Heading(product.title)
            Text(
                decimalMoney((chosen ?: product).price, product.currency),
                color = Vermilion,
                fontSize = 28.sp,
                fontWeight = FontWeight.Bold,
            )
                        ...
                        label = {
                            Text(
                                variant.option_values.values.joinToString(" / ").ifBlank {
                                    variant.title
                                } + " · " + decimalMoney(variant.price, variant.currency)
                            )
                        },
                    )
                }
            }
            product.specs.forEach { (name, value) ->
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Note(name)
                    Text(value, Modifier.widthIn(max = 230.dp))
                }
            }
            Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                Button(
                    onClick = { chosen?.let(vm::add) },
                    enabled = chosen?.in_stock == true && !writing,
                    modifier = Modifier.weight(1f),
                ) {
                    Text(if (chosen == null) "请先选规格" else "加入购物车")
                }
            }
        }
    }
...
@Composable
fun ProductImage(url: String?, title: String, modifier: Modifier = Modifier) {
    SubcomposeAsyncImage(
        ...
        loading = {
            Box(Modifier.fillMaxSize().background(Line), contentAlignment = Alignment.Center) {
                CircularProgressIndicator(Modifier.size(20.dp), strokeWidth = 2.dp)
            }
        },
        error = {
            Column(
                Modifier.fillMaxSize().background(Line),
                horizontalAlignment = Alignment.CenterHorizontally,
                verticalArrangement = Arrangement.Center,
            ) {
                Icon(Icons.Outlined.Inventory2, null, Modifier.size(42.dp), tint = Muted)
