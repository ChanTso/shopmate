...
def build_buyer_agent(settings: Settings, backend, provider: Provider, *, memory_store):
    ...
    config = ShoppingAgentConfig(
        ...
        domain_search_notes=(
            "商品搜索按目录文字匹配，不会自动翻译或理解用途；不要预设商品名只使用英文。"
            "先用用户语言中的简短商品词，按每种用途分别检索；中文请求先查中文品类词，如咖啡机。"
            "类别和属性筛选只用已读取的目录值；有货要求检查商品返回的in_stock，不虚构availability属性。"
            "无结果时去掉猜测的筛选，缩短关键词并尝试中英文同义词，再判断是否未找到；给用户的解释仍用中文。"
