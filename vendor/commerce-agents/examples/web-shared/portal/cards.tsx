...
interface ChangeLike {
  change_id: string;
  status: "staged" | "applied" | "discarded" | "rejected";
...
export const CHANGE_STATUS: Record<ChangeLike["status"], { tone: Tone; label: string }> = {
  ...
  applied: { tone: "ok", label: "已执行" },
  discarded: { tone: "muted", label: "已取消" },
  rejected: { tone: "danger", label: "整批拒绝" },
};
...
export function useChangeActions<T extends ChangeLike>(
  ...
  const act = async (action: ChangeAction) => {
    ...
    try {
      ...
      else setError("未取得执行结果，请刷新草案后重试。");
    } catch (error) {
      setError(error instanceof Error ? error.message : "操作失败，请刷新草案后重试。");
    } finally {
      setBusy(null);
    }
  };
...
/** Approval is an explicit operator action; chat cannot authorize writes. */
      ...
      {change.status === "staged" ? (
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="accent" size="sm" icon="check" onClick={() => onAct("apply")} disabled={busy !== null || !canAct}>
            {busy === "apply" ? "执行中…" : "批准并执行"}
          </Button>
          <Button variant="secondary" size="sm" onClick={() => onAct("discard")} disabled={busy !== null || !canAct}>
            {busy === "discard" ? "取消中…" : "取消草案"}
          </Button>
          <span className="text-[11.5px] leading-tight text-(--ink-soft)">批准后将按草案价格执行整批变更。</span>
        </div>
      ) : (
        <div className="flex items-center gap-2 text-[13px] text-(--ink-soft)">
          <Icon name={change.status === "applied" ? "check" : "x"} size={15} className={change.status === "applied" ? "text-(--ok)" : "text-(--ink-faint)"} />
          {change.status === "applied"
            ? `已执行${change.applied_at ? ` · ${formatDate(change.applied_at)}` : ""}`
            : change.status === "rejected" ? "整批未执行，请查看拒绝原因。" : "草案已取消，商品价格未变更。"}
        </div>
      )}
