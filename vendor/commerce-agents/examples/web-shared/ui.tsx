...
export function StatTile({
      ...
      <div className="relative text-[12.5px] font-medium whitespace-nowrap text-(--ink-soft)">
        ...
        {onClick ? (
          <span className="absolute right-0 top-0 flex items-center gap-1 bg-inherit text-[11.5px] text-(--ink-faint) opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100">
            查看原因 <Icon name="arrow-right" size={12} />
          </span>
        ) : null}
      </div>
      <div className="mt-1.5 flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1.5">
        <span className="min-w-0 max-w-full text-[26px] font-semibold leading-tight tracking-[-0.02em] tabular-nums text-(--ink) [overflow-wrap:anywhere]">{value}</span>
  ...
  const className = "group block min-w-0 w-full px-[18px] pb-3.5 pt-4 text-left transition-colors";
  return onClick ? (
...
export function StatStrip({ children }: { children: ReactNode }) {
  return (
    <div className="@container overflow-hidden rounded-2xl">
      <div className="grid grid-cols-1 gap-px bg-(--line) @[440px]:grid-cols-2 @[960px]:grid-cols-4 [&>*]:bg-(--card)">
        {children}
      </div>
    </div>
  );
