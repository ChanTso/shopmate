...
import { formatTurnUsage } from "./usage";
...
export function useAgentTurn(api: AgentApi, options: AgentTurnOptions): AgentTurn {
  ...
  const [items, setItems] = useState<ChatItem[]>(options.initialItems ?? []);
  ...
  const turnRef = useRef(Math.max(0, ...(options.initialItems ?? []).map((item) => item.kind === "assistant" ? item.turn : 0)));
  ...
  const runTurn = useCallback(
    async (userText: string, events: AsyncIterable<AgentEvent>) => {
      ...
      try {
        for await (const event of events) {
          if (event.type === "turn_complete") {
            ...
            appendTrace({
              ...
              turn,
              label: "运行完成",
              detail: formatTurnUsage(event.data),
        ...
        const text = error instanceof Error ? error.message : unreachable;
        updateTurn(turn, (item) => ({ ...item, segments: [...item.segments, { type: "error", text }] }));
      } finally {
