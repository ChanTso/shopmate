...
export async function* readEventStream(body: ReadableStream<Uint8Array>): AsyncGenerator<AgentEvent> {
...
/** A reported server failure is a terminal reply; only an unexplained EOF is a disconnect. */
export async function* readChatEventStream(body: ReadableStream<Uint8Array>): AsyncGenerator<AgentEvent> {
  let terminal = false;
  for await (const event of readEventStream(body)) {
    if (event.type === "turn_complete" || event.type === "error") terminal = true;
    yield event;
  }
  if (!terminal) throw new Error("连接已中断。已保存的草案可在刷新会话后查看，请勿凭未完成的回复判断执行结果。");
}
