...
def build_static_system(config: MerchantAgentConfig, skills: SkillRegistry) -> str:
    ...
    if config.enable_pricing and "stage_promotion" in config.absent_tools():
        absent_names.append("promotions")
    absent_rule = (
    ...
    if stages and "apply_change" in config.absent_tools():
        change_contract = (
            "\n- Stage only the exact product and price changes requested. The preview is a "
            "proposal, not an execution. Only the operator's approval button calls the "
            "business transaction; chat approval and model tools cannot execute a proposal. "
            "After approval, read the actual product and stored receipt before claiming success."
            "\n- Each proposal contains at most the configured item count, in one currency. "
            "Ask the operator to narrow an over-limit request instead of silently splitting it."
        )
        staging_tool_rules = (
            "\n- Read target listings with search_listings or get_listing before staging. "
            "Stage tools create drafts only. Do not invent an apply tool or an approval result. "
            "A rejected or cancelled draft leaves the live prices unchanged."
        )
        one_call_examples = "one metric, one listing record, reading an approval result"
    second_job = (
