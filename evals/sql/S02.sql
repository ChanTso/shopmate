...
SELECT product_id, SUM(total_price_minor) AS amount_minor,
       ROUND(100 * SUM(total_price_minor)
         / SUM(SUM(total_price_minor)) OVER (), 2) AS share_pct
FROM paid
WHERE currency = 'CNY' AND succeeded_at >= '2026-08-22'
  AND succeeded_at < '2026-09-05'
GROUP BY product_id ORDER BY amount_minor DESC, product_id LIMIT 3;
