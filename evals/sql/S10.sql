...
SELECT COUNT(*) AS paid_orders, COALESCE(SUM(quantity), 0) AS units,
       COALESCE(SUM(total_price_minor), 0) AS amount_minor
FROM paid WHERE currency = 'CNY'
  AND succeeded_at >= '2026-07-01' AND succeeded_at < '2026-07-25';
