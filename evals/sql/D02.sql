...
SELECT CASE WHEN succeeded_at < '2026-08-22' THEN 'prior14' ELSE 'recent14' END AS period, COUNT(*) AS orders, SUM(quantity) AS units, SUM(total_price_minor) AS amount_minor FROM paid WHERE currency='CNY' AND succeeded_at >= '2026-08-08' AND succeeded_at < '2026-09-05' GROUP BY period ORDER BY period;
