...
SELECT currency,
       ...
       COUNT(*) AS orders, SUM(quantity) AS units, SUM(total_price_minor) AS amount_minor
FROM paid WHERE succeeded_at >= '2026-08-08' AND succeeded_at < '2026-09-05'
GROUP BY currency, period ORDER BY currency, period DESC;
