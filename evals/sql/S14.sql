...
SELECT product_id,
       CASE WHEN succeeded_at < '2026-09-01' THEN 'Aug28-31' ELSE 'Sep1-4' END AS period,
       COUNT(DISTINCT DATE(succeeded_at)) AS active_days,
       SUM(quantity) AS units, SUM(total_price_minor) AS amount_minor
FROM paid WHERE currency = 'CNY'
  AND succeeded_at >= '2026-08-28' AND succeeded_at < '2026-09-05'
GROUP BY product_id, period ORDER BY product_id, period;
