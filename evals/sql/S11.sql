...
SELECT product_id, SUM(quantity) AS units, SUM(total_price_minor) AS amount_minor,
       DENSE_RANK() OVER (ORDER BY SUM(quantity) DESC) AS units_rank,
       DENSE_RANK() OVER (ORDER BY SUM(total_price_minor) DESC) AS amount_rank
FROM paid WHERE currency = 'CNY'
  AND succeeded_at >= '2026-08-22' AND succeeded_at < '2026-09-05'
GROUP BY product_id ORDER BY units_rank, product_id;
