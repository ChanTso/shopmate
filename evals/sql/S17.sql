...
SELECT o.product_id, p.stock_quantity, SUM(o.total_price_minor) AS amount_minor
FROM paid o JOIN product p USING (product_id)
WHERE o.currency = 'CNY' AND p.stock_quantity >= 50
  AND succeeded_at >= '2026-08-22' AND succeeded_at < '2026-09-05'
GROUP BY o.product_id, p.stock_quantity ORDER BY amount_minor DESC, o.product_id;
