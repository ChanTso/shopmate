...
SELECT p.product_id, p.stock_quantity, COUNT(o.order_id) AS paid_orders,
       COALESCE(SUM(o.quantity), 0) AS units,
       COALESCE(SUM(CASE WHEN o.succeeded_at >= '2026-08-08'
                         AND o.succeeded_at < '2026-08-22'
                        THEN o.total_price_minor ELSE 0 END), 0) AS b_amount_minor,
       COALESCE(SUM(CASE WHEN o.succeeded_at >= '2026-08-22'
                        THEN o.total_price_minor ELSE 0 END), 0) AS a_amount_minor
FROM product p LEFT JOIN paid o ON o.product_id = p.product_id
  AND o.succeeded_at >= '2026-07-25' AND o.succeeded_at < '2026-09-05'
WHERE p.product_id = 'shopmate-fixture-tote'
