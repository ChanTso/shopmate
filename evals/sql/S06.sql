...
SELECT DATE(succeeded_at) AS sale_date, SUM(quantity) AS units,
       SUM(total_price_minor) AS amount_minor
FROM paid WHERE product_id = 'shopmate-fixture-mug'
  AND succeeded_at >= '2026-08-22' AND succeeded_at < '2026-09-05'
GROUP BY DATE(succeeded_at) ORDER BY sale_date;
