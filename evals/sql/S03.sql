...
SELECT SUM(quantity) AS units, SUM(total_price_minor) AS amount_minor,
       SUM(total_price_minor) / SUM(quantity) AS paid_unit_price_minor,
       MAX(p.price_minor) AS current_price_minor
FROM paid o JOIN product p USING (product_id)
WHERE o.product_id = 'shopmate-fixture-coffee'
