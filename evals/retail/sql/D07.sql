...
SELECT 'prior14' AS period, COUNT(*) AS orders, COALESCE(SUM(quantity),0) AS units, COALESCE(SUM(total_price_minor),0) AS amount_minor FROM paid WHERE product_id='AR-1004' AND currency='CNY' AND succeeded_at >= '2026-08-07 16:00:00' AND succeeded_at < '2026-08-21 16:00:00'
UNION ALL SELECT 'recent14',COUNT(*),COALESCE(SUM(quantity),0),COALESCE(SUM(total_price_minor),0) FROM paid WHERE product_id='AR-1004' AND currency='CNY' AND succeeded_at >= '2026-08-21 16:00:00' AND succeeded_at < '2026-09-04 16:00:00';
